...
const Blog = ({ blog, updateBlog, deleteBlog }) => {
	...
	if (blogDetailsVisible) {
		return (
			<div style={blogStyle} className="blog2">
				...
				{blog.url} <br/>
				{blog.likes} <button className="like" onClick={increaseLike}>like</button><br/>
				{blog.user.name}<br/>
	...
	return (
		<div style={blogStyle} className='blog'>
			<div>
				<p> {blog.title} {blog.author} <button className="view"onClick={() => setBlogDetailsVisible(true)}>view</button></p>
			</div>
		</div>
	)
